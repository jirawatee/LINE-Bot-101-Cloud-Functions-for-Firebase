...
const LINE_CHANNEL_SECRET = functions.config().lineoa.jirawatee.channel_secret;
const LINE_HEADER = {
  "Content-Type": "application/json",
  Authorization: `Bearer ${functions.config().lineoa.jirawatee.channel_access_token}`
};
...
const postToDialogflow = async (req) => {
  ...
  return axios({
    url: `https://dialogflow.cloud.google.com/v1/integrations/line/webhook/${functions.config().dialogflow.agent_id}`,
    headers: req.headers,
    method: "post",
    data: req.body
  });
